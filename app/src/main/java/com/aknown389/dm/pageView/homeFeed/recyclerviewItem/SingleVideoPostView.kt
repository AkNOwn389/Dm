...
class SingleVideoPostView @Inject constructor(
    ...
    private fun setUI(){
        ...
        if (currentItem.NoOfcomment == 0){
            ...
            holder.noOfComments?.text = currentItem.NoOfcomment.toString()+context.getString(R.string.comments)
        ...
        try {
            ...
            val height = getVideoHeight(currentItem)
        ...
        if (holder.thumbnail != null && currentItem.videos != null){
            requestManager.load(currentItem.videos)
                .override(700, 700)
                .into(holder.thumbnail!!).toString()
        }
        when(currentItem.privacy){
            'P' -> holder.privacy?.text = parent.context.getString(R.string.fublic)
            'F' -> holder.privacy?.text = parent.context.getString(R.string.friendsSmall)
            'O' -> holder.privacy?.text = parent.context.getString(R.string.only_me)
        }
        setVideoPlayer(holder, currentItem)

    }
    private fun getVideoHeight(data: PostDataModel): Int {
        ...
        if (data.width!! > width) {
            val num: Double = (data.width / width).toDouble()
            val height: Int? = data.height?.div(num)?.toInt()
            return height!!
        }else{
            val num = width-data.width
            val num2 = data.height?.plus(num)
            return num2!!
        }
    }
    ...
    private fun setVideoPlayer(holder: HomeFeedRecyclerViewHolder, data: PostDataModel) {
        ...
        try {
            ...
            val url = getMediaUrl(data)
            val mediaItem = MediaItem.Builder()
                .setUri(url)
                .build()
            exoPlayer.setMediaItem(mediaItem)
            exoPlayer.seekTo(data.videoPlayerPlaybackPosition)
        }catch (e:NullPointerException){
            Toast.makeText(context, "On adapter $e", Toast.LENGTH_SHORT).show()
        }
    }
